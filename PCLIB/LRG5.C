 ...
 */
lrg_arc(int x, int y, int r, unsigned char v1, unsigned char v2, int c)
{
    ...
    do {
        ...
        switch(v1 & (ARC_RES*3)) {
            case ARC_RES*0 :    /* Quadrant one */
                x1 = x + (ax = lrg_scale(r, sine[i], -1));
                y1 = y - sqrt(rs - (ax*ax));
                break;
            case ARC_RES*1 :    /* Quadrant two */
                x1 = x + (ax = lrg_scale(r, sine[j], -1));
                y1 = y + sqrt(rs - (ax*ax));
                break;
            ...
            case ARC_RES*3 :    /* Quadrant four */
                ...
                y1 = y - sqrt(rs - (ax*ax)); }
        if(x2 != -1)
            lrg_line(x2, (y2<<2)/5, x1, (y1<<2)/5, c);
        x2 = x1;
        y2 = y1; }
    while(v1++ != v2);
}
